...
const createEntries = async (entryData: EntryData) => {
    const response = await fetch('http://localhost:3300/entry', {
        method: 'POST',
        headers: {
...
const createEntriesJson = async (formData: FormData) => {
    const response = await fetch('http://localhost:3300/entry/json', {
        method: 'POST',
        body: formData,
    });
    if (!response.ok) {
...
const getEntries = async (leaderboardId: string) => {
    const response = await fetch(`http://localhost:3300/entry/${leaderboardId}`);
    if (!response.ok) {
